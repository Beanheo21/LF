...
with tab1:
    ...
    if SIM_AVAILABLE:
        ...
        # ── Middle East Oil Panel ────────────────────────────────
        st.markdown("#### 🛢️ Middle East Oil & Energy Context")

        MIDDLE_EAST_OIL = {
            "COVID-19 Pandemic": {
                "relevance": "Low direct impact",
                "color": "#22c55e",
                "detail": "Oil demand collapsed by ~30% in 2020 as lockdowns halted global transport and manufacturing. OPEC+ cut production by a record 9.7M barrels/day to stabilize prices. Saudi Aramco and UAE ADNOC maintained output capacity but faced historic price crashes — WTI briefly went negative in April 2020.",
                "key_stat": "Oil price: -7/barrel (April 2020)",
                "countries": "Saudi Arabia, UAE, Iraq, Kuwait, Iran",
            },
            "Suez Canal Blockage": {
                "relevance": "High — critical chokepoint for Gulf oil",
                "color": "#ef4444",
                "detail": "12% of global trade passes through the Suez Canal, including ~1M barrels of oil per day from the Persian Gulf to Europe. The 6-day blockage forced tankers to reroute around Africa's Cape of Good Hope — adding 9–12 days and ~00,000 extra fuel per voyage. Brent crude spiked 6% within 48 hours of the incident.",
                "key_stat": "~1M barrels/day rerouted around Africa",
                "countries": "Egypt (canal), Saudi Arabia, UAE, Qatar",
            },
            "1973 Oil Embargo": {
                "relevance": "Origin event — maximum impact",
                "color": "#ef4444",
                "detail": "OPEC members (Saudi Arabia, Iraq, Iran, Kuwait, Libya, UAE) cut oil exports to the US, UK, Canada, Japan, and Netherlands. This was a direct geopolitical weapon. Oil prices rose from  to 2/barrel (300%) in months. The US introduced gasoline rationing, odd/even license plate filling days, and a national 55mph speed limit. It permanently reshaped Western energy policy.",
                "key_stat": "Oil price:  → 2/barrel (+300%)",
                "countries": "Saudi Arabia, Iraq, Libya, Kuwait, UAE, Iran",
            },
            "Semiconductor Shortage": {
                "relevance": "Moderate — energy input costs",
                "color": "#f59e0b",
                "detail": "Semiconductor fabrication is extraordinarily energy-intensive — TSMC alone consumes ~5% of Taiwan's total electricity. During the chip shortage, Middle Eastern sovereign wealth funds (Saudi PIF, UAE Mubadala) began investing heavily in chip companies and US fab construction as part of broader tech diversification strategies away from oil dependency.",
                "key_stat": "TSMC uses ~5% of Taiwan's total electricity",
                "countries": "Saudi Arabia (PIF), UAE (Mubadala), Qatar (QIA)",
            },
            "Russia-Ukraine Sanctions": {
                "relevance": "Very High — Gulf fills European energy gap",
                "color": "#ef4444",
                "detail": "When Europe cut Russian energy imports, Gulf states became critical swing suppliers. Qatar dramatically increased LNG exports to Europe. Saudi Arabia and UAE refused to increase OPEC production quotas despite US pressure, effectively maintaining elevated prices. This gave Gulf states unprecedented geopolitical leverage as Russia-aligned and Western nations competed for their oil.",
                "key_stat": "Qatar LNG exports to Europe +40% in 2022",
                "countries": "Qatar, Saudi Arabia, UAE, Kuwait",
            },
            "Taiwan Strait Scenario": {
                "relevance": "Critical — Gulf oil funds Taiwan defense budgets",
                "color": "#f59e0b",
                "detail": "A Taiwan conflict would immediately spike global oil prices as markets priced in potential US-China confrontation. The Strait of Hormuz (controlled near Iran) and the Malacca Strait (between the South China Sea conflict zone and the Persian Gulf) would both face closure risk simultaneously. Scenario models suggest oil could reach 00+/barrel, collapsing the global economy before any military resolution.",
                "key_stat": "Hormuz + Malacca closure = ~40% of global oil blocked",
                "countries": "Saudi Arabia, UAE, Iran, Qatar, Kuwait",
            },
        }

        me = MIDDLE_EAST_OIL.get(event_name, list(MIDDLE_EAST_OIL.values())[0])

        col_me1, col_me2 = st.columns([2, 1])
        with col_me1:
            st.markdown(
                f"<div class='event-card' style='border-left-color:{me['color']}'>"                f"<div class='event-title'>🛢️ Gulf Oil Impact: {me['relevance']}</div>"                f"<div class='event-desc' style='margin-top:8px'>{me['detail']}</div>"                f"</div>",
                unsafe_allow_html=True
            )
        with col_me2:
            st.markdown(
                f"<div class='metric-card' style='border-color:{me['color']}44; text-align:left'>"                f"<div class='metric-label'>Key Statistic</div>"                f"<div style='font-family:JetBrains Mono,monospace; font-size:14px; color:{me['color']}; margin:8px 0'>{me['key_stat']}</div>"                f"<div class='metric-label' style='margin-top:12px'>Countries Involved</div>"                f"<div style='font-size:12px; color:#8899b4; margin-top:4px'>{me['countries']}</div>"                f"</div>",
                unsafe_allow_html=True
            )

        # Middle East oil map
        me_nodes = [
            ("Saudi Arabia",   {"lat": 24.7,  "lon": 46.7,  "oil_mb": 10.5, "role": "Largest OPEC producer"}),
            ("UAE",            {"lat": 24.5,  "lon": 54.4,  "oil_mb": 4.0,  "role": "OPEC+ swing producer"}),
            ("Iraq",           {"lat": 33.3,  "lon": 44.4,  "oil_mb": 4.5,  "role": "2nd largest OPEC producer"}),
            ("Iran",           {"lat": 35.7,  "lon": 51.4,  "oil_mb": 3.8,  "role": "Sanctioned producer"}),
            ("Kuwait",         {"lat": 29.4,  "lon": 47.9,  "oil_mb": 2.7,  "role": "High reserve-to-production ratio"}),
            ("Qatar",          {"lat": 25.3,  "lon": 51.2,  "oil_mb": 1.8,  "role": "World largest LNG exporter"}),
            ("Strait Hormuz",  {"lat": 26.6,  "lon": 56.3,  "oil_mb": 21.0, "role": "20% of global oil passes here"}),
            ("Suez Canal",     {"lat": 30.5,  "lon": 32.3,  "oil_mb": 5.5,  "role": "Key Asia-Europe oil route"}),
            ("Bab-el-Mandeb",  {"lat": 12.6,  "lon": 43.3,  "oil_mb": 6.2,  "role": "Red Sea chokepoint"}),
        ]
        me_edges = [
            ("Saudi Arabia",  "Strait Hormuz"),
            ("UAE",           "Strait Hormuz"),
            ("Iraq",          "Strait Hormuz"),
            ("Iran",          "Strait Hormuz"),
            ("Kuwait",        "Strait Hormuz"),
            ("Qatar",         "Strait Hormuz"),
            ("Strait Hormuz", "Bab-el-Mandeb"),
            ("Bab-el-Mandeb", "Suez Canal"),
        ]

        fig_me = go.Figure()

        for src, dst in me_edges:
            s = dict(me_nodes)[src]; d = dict(me_nodes)[dst]
            fig_me.add_trace(go.Scattergeo(
                lat=[s["lat"], d["lat"]],
                lon=[s["lon"], d["lon"]],
                mode="lines",
                line=dict(width=2, color="rgba(251,146,60,0.5)"),
                hoverinfo="none", showlegend=False,
            ))

        for name, attrs in me_nodes:
            size = min(8 + attrs["oil_mb"] * 1.5, 30)
            fig_me.add_trace(go.Scattergeo(
                lat=[attrs["lat"]],
                lon=[attrs["lon"]],
                mode="markers+text",
                marker=dict(size=size, color="#fb923c",
                            line=dict(color="#04060f", width=1.5),
                            opacity=0.85),
                text=[name],
                textposition="top center",
                textfont=dict(size=9, color="#fed7aa", family="JetBrains Mono"),
                hovertemplate=(
                    f"<b>{name}</b><br>"
                    f"Output: {attrs['oil_mb']}M barrels/day<br>"
                    f"Role: {attrs['role']}<extra></extra>"
                ),
                showlegend=False,
            ))
        ...
        st.caption("Node size = daily oil output (million barrels). Strait of Hormuz and Suez Canal shown as critical chokepoints.")
...
with tab6:
    ...
    st.markdown(
        ...
        "margin-bottom:20px'>Real-time geopolitical & trade news from Reuters, BBC, FT, "
        "and AP. Auto-refreshes every 5 minutes. Risk-tagged by AI keyword matching.</div>",
        unsafe_allow_html=True
    )

    # ── Risk keyword classifier ───────────────────────────────────
    RISK_KEYWORDS = {
        "CRITICAL": ["war","invasion","nuclear","blockade","sanctions","embargo","attack","missile","conflict","bombing","siege"],
        "HIGH":     ["shortage","disruption","port closure","strike","protest","coup","tariff","inflation","crisis","recession","drought","flood"],
        "MEDIUM":   ["tension","supply chain","freight","shipping","delay","slowdown","trade","commodity","oil","energy","weather"],
        "LOW":      ["agreement","deal","recovery","growth","investment","diplomacy","ceasefire","export","import"],
    }
    ...
    # ── News feeds (RSS — no API key needed) ─────────────────────
    FEEDS = [
        {"name":"Reuters",  "url":"https://feeds.reuters.com/reuters/businessNews", "color":"#ff8c00"},
        {"name":"BBC",      "url":"https://feeds.bbci.co.uk/news/business/rss.xml", "color":"#cc0000"},
        {"name":"AP",       "url":"https://rsshub.app/apnews/topics/business",      "color":"#2563eb"},
        {"name":"FT",       "url":"https://www.ft.com/world?format=rss",            "color":"#f59e0b"},
    ]

    SUPPLY_CHAIN_TERMS = [
        "supply chain","trade","port","shipping","freight","sanction","embargo",
        "oil","energy","semiconductor","chip","inflation","tariff","conflict",
        "war","shortage","logistics","cargo","import","export","commodity",
        "ukraine","russia","china","taiwan","red sea","suez","gaza","iran",
        "opec","fed","recession","drought","strike","geopolit",
    ]
    ...
    def fetch_feed(feed):
        ...
        try:
            req = urllib.request.Request(
                feed["url"],
                headers={"User-Agent": "Mozilla/5.0 (supply-chain-dashboard/1.0)"}
            )
            ...
            for item in items[:12]:
                ...
                if any(term in combined for term in SUPPLY_CHAIN_TERMS):
    ...
    filtered = [
        ...
        if a["risk"] in filter_risk and a["source"] in filter_source
    ]

    st.markdown(
        f"<div style='font-family:JetBrains Mono,monospace;font-size:11px;"
        f"color:#4a5a78;margin-bottom:16px;'>"
        f"Showing {len(filtered)} articles</div>",
        unsafe_allow_html=True
    )
